...
impl CommunicationChannel for TcpChannel {
    ...
    fn send_16byte_block<const N: usize>(&mut self, data: &[[u128; N]]) -> std::io::Result<u64> {
        let size = data.len() as u64;
        ...
        for block in data {
            for elem in block {
                self.stream
                    .write_all(&elem.to_le_bytes())?;
            }
        }

        Ok(size * (N as u64) * 16)  // Return the number of bytes sent
    }

    /// Receives an array of 16-byte blocks over the TCP channel.
    fn receive_16byte_block<const N: usize>(&mut self) -> std::io::Result<Vec<[u128; N]>> {
        let mut size_buf = [0u8; 8];
        self.stream
            .read_exact(&mut size_buf)?;
        let size = u64::from_le_bytes(size_buf) as usize;
        ...
        for _ in 0..size {
            ...
            for elem in &mut block {
                let mut elem_buf = [0u8; 16];
                self.stream
                    .read_exact(&mut elem_buf)?;
                *elem = u128::from_le_bytes(elem_buf);
            }
            data.push(block);
        }

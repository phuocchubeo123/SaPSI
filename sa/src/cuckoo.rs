...
impl<const NUM_LIMBS: usize> CuckooHash<NUM_LIMBS> {
    ...
    pub fn new(table_size: usize, max_probe: usize) -> Self {
        Self {
            table: vec![[0u128; NUM_LIMBS]; table_size],
            loc_funcs: vec![],
    ...
    pub fn insert(&mut self, item: &[u128; NUM_LIMBS]) -> bool {
        if self.query(item) {
            return false;
        }

        let mut level: usize = self.max_probe;
        let mut curr_item = item.clone();
        while level > 0 {
            // Loop over all posible locations
            for location in self.all_locations(item).iter() {
                if self.table[*location] == [0u128; NUM_LIMBS] {
            ...
            let swap_location = self.location(item, rand_loc_func_index);
            
            let mut temp = self.table[swap_location];
            self.table[swap_location] = *item;
            curr_item = temp;
            level -= 1;
        }
